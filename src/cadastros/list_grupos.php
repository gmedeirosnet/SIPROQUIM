...
if (isset($_POST['delete']) && isset($_POST['id'])) {
    ...
    try {
        ...
        $stmt_check = $pdo->prepare("SELECT COUNT(*) FROM produtos WHERE id_grupo = :id");
        $stmt_check->execute([':id' => $id]);
        $produtos_count = $stmt_check->fetchColumn();

        if ($produtos_count > 0) {
            $error = "Não é possível excluir este grupo pois existem produtos associados a ele.";
        } else {
...
<body>
    <div class="container">
        ...
        <?php if (count($grupos) > 0): ?>
                ...
                <tbody>
                    <?php foreach ($grupos as $grupo): ?>
                        <tr>
                            ...
                            <td class="actions">
                                <a href="grupo.php?id=<?= $grupo['id'] ?>" class="btn btn-warning">Editar</a>
                                <form method="post" onsubmit="return confirm('Tem certeza que deseja excluir este grupo?');" style="display: inline;">
                                    <input type="hidden" name="id" value="<?= $grupo['id'] ?>">
                                    <button type="submit" name="delete" class="btn btn-danger" <?= $grupo['total_produtos'] > 0 ? 'disabled title="Não é possível excluir um grupo que está sendo usado"' : '' ?>>Excluir</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
            ...
            <?php endif; ?>
        <?php else: ?>
            <p>Nenhum grupo de produtos encontrado.</p>
        <?php endif; ?>
