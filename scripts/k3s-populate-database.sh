...
CREATE TABLE IF NOT EXISTS movimentos (id SERIAL PRIMARY KEY, id_produto INTEGER REFERENCES produtos(id) ON DELETE CASCADE, id_pessoa INTEGER REFERENCES pessoas(id) ON DELETE CASCADE, id_lugar INTEGER REFERENCES lugares(id) ON DELETE CASCADE, tipo VARCHAR(15) NOT NULL, quantidade INTEGER NOT NULL, data_movimento TIMESTAMP DEFAULT CURRENT_TIMESTAMP, observacao TEXT, id_lugar_destino INTEGER REFERENCES lugares(id) ON DELETE SET NULL);
